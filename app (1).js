...
class PixelArtApp {
    ...
    async processWithdraw() {
        try {
            ...
            if (!amount || amount < 50 || amount > (this.currentUser?.balance || 0)) {
                throw new Error(`المبلغ يجب أن يكون بين 50 و ${this.currentUser?.balance || 0} ريال`);
            }
            ...
            if (method === 'bank') {
                ...
                if (!bankName || !accountNumber) {
                    throw new Error('يرجى إدخال جميع بيانات الحساب البنكي');
                }
                details = `إلى ${bankName} - ${accountNumber}`;
            } else {
                const stcNumber = document.getElementById('stcNumber')?.value;
                if (!stcNumber) {
                    throw new Error('يرجى إدخال رقم STC Pay');
                }
                details = `إلى STC Pay: ${stcNumber}`;
            }
            ...
            this.addTransaction({
                type: 'withdrawal',
                amount: -amount,
                description: `سحب ${details}`,
                status: 'completed'
            });
            
            this.closeModal('withdrawModal');
            this.showSuccess('تم السحب', `تم سحب ${amount} ريال بنجاح. سيصل إلى حسابك خلال 24-48 ساعة`);
            ...
            <div class="modal-body">
                <form id="addPaymentMethodForm">
                    <div class="payment-method-types">
                        <div class="method-type-card selected" data-type="card">
                    ...
                    <div class="form-content" id="cardForm">
                        <div class="form-group">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" id="cardCVV" placeholder="123" maxlength="4" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>اسم حامل البطاقة *</label>
                            <input type="text" id="cardHolder" placeholder="${this.currentUser?.name || 'اسم حامل البطاقة'}" required>
                        </div>
                    </div>
                    
                    <div class="form-content" id="bankForm" style="display: none;">
                        <div class="form-group">
                            <label>اسم البنك *</label>
                            <input type="text" id="bankName" placeholder="البنك الأهلي السعودي" required>
                        </div>
                        <div class="form-group">
                            <label>رقم الحساب (IBAN) *</label>
                            <input type="text" id="ibanNumber" placeholder="SA44 2000 0001 2345 6789 1234" required>
                        </div>
                        <div class="form-group">
                            <label>اسم صاحب الحساب *</label>
                            <input type="text" id="accountHolder" placeholder="${this.currentUser?.name || 'اسم صاحب الحساب'}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="checkbox-label">
                            ...
                            <span>جعل هذه الطريقة الافتراضية</span>
                        </label>
                    </div>
                </form>
            </div>
    ...
    async savePaymentMethod() {
        try {
            const type = document.querySelector('.method-type-card.selected').dataset.type;
            const isDefault = document.getElementById('makeDefault').checked;
            ...
            if (type === 'card') {
                ...
                // التحقق من صحة رقم البطاقة (مبسط)
                const cleanNumber = cardNumber.replace(/\s/g, '');
                if (cleanNumber.length !== 16) {
                    throw new Error('رقم البطاقة يجب أن يكون 16 رقماً');
                }
                
                paymentMethod = {
                    type: 'visa',
                    name: 'بطاقة Visa/Mastercard',
                    number: `•••• •••• •••• ${cleanNumber.substring(12)}`,
                    expiry: cardExpiry,
                    holder: cardHolder,
                    isDefault: isDefault
                };
                ...
                if (!bankName || !ibanNumber || !accountHolder) {
                ...
                paymentMethod = {
                    type: 'bank',
                    name: bankName,
                    number: `•••• •••• ${ibanNumber.substring(ibanNumber.length - 4)}`,
            ...
            paymentMethod.userId = this.currentUser?.id;
            ...
            this.showSuccess('تمت الإضافة', 'تمت إضافة طريقة الدفع بنجاح');
    ...
    editPaymentMethod(methodId) {
        ...
        const modalContent = `
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="app.closeModal('editPaymentMethodModal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <form id="editPaymentMethodForm">
                    <div class="current-method-info">
                        ...
                        <div>
                            <h4>${method.name}</h4>
                            <p>${method.number}</p>
                            ${method.expiry ? `<p>تنتهي في: ${method.expiry}</p>` : ''}
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label class="checkbox-label">
                            <input type="checkbox" id="editMakeDefault" ${method.isDefault ? 'checked' : ''}>
                            <span>جعل هذه الطريقة الافتراضية</span>
                        </label>
                    </div>
                    
                    <div class="form-group">
                        <button type="button" class="action-btn outline danger" onclick="app.confirmDeletePaymentMethod(${methodId})">
                            <i class="fas fa-trash"></i>
                            حذف طريقة الدفع
                        </button>
                    </div>
            ...
            <div class="modal-footer">
                <button class="action-btn outline" onclick="app.closeModal('editPaymentMethodModal')">
                    إلغاء
                </button>
                <button class="action-btn" type="submit" form="editPaymentMethodForm">
                    <i class="fas fa-save"></i>
                    حفظ التغييرات
                </button>
            </div>
        ...
        this.createModal('editPaymentMethodModal', modalContent);
        ...
        setTimeout(() => {
            ...
            if (form) {
                form.addEventListener('submit', async (e) => {
                    e.preventDefault();
                    await this.updatePaymentMethod(methodId);
                });
            }
        }, 100);
        
        this.openModal('editPaymentMethodModal');
    }
    
    async updatePaymentMethod(methodId) {
        try {
            const isDefault = document.getElementById('editMakeDefault').checked;
            ...
            if (isDefault) {
                ...
                // تحديث الطريقة المحددة
                paymentMethodsList = paymentMethodsList.map(method => 
                    method.id === methodId ? { ...method, isDefault } : method
                );
            }
            ...
            this.showSuccess('تم التحديث', 'تم تحديث طريقة الدفع بنجاح');
    ...
    deletePaymentMethod(methodId) {
        try {
            ...
            // التحقق مما إذا كانت الطريقة هي الافتراضية الوحيدة
            const method = paymentMethodsList.find(m => m.id === methodId);
            const isDefault = method?.isDefault;
            const hasOtherDefault = paymentMethodsList.some(m => 
                m.id !== methodId && m.isDefault
            );
            
            // حذف الطريقة
            paymentMethodsList = paymentMethodsList.filter(m => m.id !== methodId);
            
            // إذا كانت الطريقة المحذوفة افتراضية ولم تكن هناك طريقة افتراضية أخرى
            if (isDefault && !hasOtherDefault && paymentMethodsList.length > 0) {
                // جعل أول طريقة افتراضية
                paymentMethodsList[0].isDefault = true;
            }
    ...
    openNewMessageModal() {
        ...
        const modalContent = `
            <div class="modal-header">
                <h3>رسالة جديدة</h3>
                <button class="modal-close" onclick="app.closeModal('newMessageModal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <form id="newMessageForm">
                    <div class="form-group">
                        ...
                        <div class="recipient-search">
                            <input type="text" id="recipientSearch" placeholder="ابحث عن مستلم...">
                            <div class="search-results" id="recipientResults" style="display: none;"></div>
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label>المشروع (اختياري)</label>
                        <select id="messageProject">
                            <option value="">اختر مشروعاً</option>
                            ${this.projects.filter(p => p.userId === this.currentUser?.id).map(p => `
                    ...
                    <div class="form-group">
                        <label>الموضوع *</label>
                        <input type="text" id="messageSubject" placeholder="موضوع الرسالة" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="file-upload-area" data-target="new-message">
                            <i class="fas fa-paperclip"></i>
                            <p>اسحب وأفلت الملفات هنا أو <span>انقر للتصفح</span></p>
                            <input type="file" multiple data-target="new-message">
                            <small>الصور، PDF، مستندات (الحد الأقصى 10MB لكل ملف)</small>
                        </div>
                        <div class="uploaded-files" id="messageUploadedFiles"></div>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="action-btn outline" onclick="app.closeModal('newMessageModal')">
                    إلغاء
                </button>
                <button class="action-btn" type="submit" form="newMessageForm">
                    <i class="fas fa-paper-plane"></i>
                    إرسال الرسالة
                </button>
            </div>
        ...
        this.createModal('newMessageModal', modalContent);
                            ...
                            if (filteredUsers.length > 0) {
                                filteredUsers.forEach(user => {
                                    ...
                                    userElement.innerHTML = `
                                        ...
                                        <div>
                                            <h5>${user.name}</h5>
                                            <p>${user.role === 'designer' ? 'مصمم' : 'عميل'}</p>
                                        </div>
                                    `;
                                    userElement.addEventListener('click', () => {
                                        searchInput.value = user.name;
                                        searchInput.dataset.selectedId = user.id;
        ...
        this.openModal('newMessageModal');
    }
    
    async sendNewMessage() {
        try {
            const recipientId = document.getElementById('recipientSearch').dataset.selectedId;
            const subject = document.getElementById('messageSubject').value;
            const content = document.getElementById('messageContent').value;
            const projectId = document.getElementById('messageProject').value;
            
            if (!recipientId) {
                throw new Error('يرجى اختيار مستلم للرسالة');
            }
            
            if (!subject || !content) {
                throw new Error('يرجى ملء الموضوع والرسالة');
            }
            ...
            const newConversation = {
                id: conversationId,
                participants: [this.currentUser?.id, parseInt(recipientId)],
                name: recipient?.name || 'مستخدم',
            ...
            // حفظ المحادثة
            const conversations = this.storage.getItem('conversations') || '[]';
            let conversationsList = JSON.parse(conversations);
            conversationsList.push(newConversation);
            this.storage.setItem('conversations', JSON.stringify(conversationsList));
            ...
            this.showSuccess('تم الإرسال', 'تم إرسال الرسالة بنجاح');
    ...
    getUserById(userId) {
        const users = this.storage.getItem('users') || '[]';
        const usersList = JSON.parse(users);
        return usersList.find(u => u.id === parseInt(userId));
    }
    ...
    searchChat() {
        ...
        if (!query) {
            this.showInfo('البحث', 'يرجى إدخال نص للبحث');
            return;
        }
        ...
        const searchResults = messagesList.filter(m => 
            m.text.toLowerCase().includes(query.toLowerCase()) ||
            m.subject?.toLowerCase().includes(query.toLowerCase())
        );
        
        if (searchResults.length === 0) {
            this.showInfo('نتائج البحث', 'لم يتم العثور على نتائج');
            return;
        }
    ...
    startVoiceCall(conversationId) {
        this.showInfo('قريباً', 'مكالمات الصوت قيد التطوير');
    }
    
    startVideoCall(conversationId) {
        this.showInfo('قريباً', 'مكالمات الفيديو قيد التطوير');
    }
    ...
    showConversationInfo(conversationId) {
        ...
        const modalContent = `
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="app.closeModal('conversationInfoModal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="conversation-info">
                    <div class="info-section">
                        ...
                        <div class="participants-list">
                            ${this.getParticipantsList(conversation)}
                        </div>
                    </div>
                    
                    ${conversation.projectId ? `
                    <div class="info-section">
                        <h4>المشروع المرتبط</h4>
                        <div class="project-info">
                            <i class="fas fa-project-diagram"></i>
                            <span>المشروع #${conversation.projectId}</span>
                        </div>
                    </div>
                    ` : ''}
                    ...
                    <div class="info-section">
                        ...
                        <div class="stats-grid">
                            <div class="stat-item">
                                <span class="stat-value">${conversation.messages?.length || 0}</span>
                                <span class="stat-label">رسائل</span>
                            </div>
                            <div class="stat-item">
                                <span class="stat-value">${this.getUnreadCount(conversation)}</span>
                                <span class="stat-label">غير مقروء</span>
                            </div>
                            <div class="stat-item">
                                <span class="stat-value">${this.formatDate(conversation.createdAt)}</span>
                    ...
                    <div class="info-section">
                        ...
                        <div class="action-buttons">
                            <button class="action-btn outline" onclick="app.clearConversation(${conversationId})">
                                <i class="fas fa-trash"></i>
                                مسح المحادثة
                            </button>
                            <button class="action-btn outline danger" onclick="app.deleteConversation(${conversationId})">
        ...
        this.createModal('conversationInfoModal', modalContent);
        this.openModal('conversationInfoModal');
    }
    
    getParticipantsList(conversation) {
        let participantsHTML = '';
        
        conversation.participants?.forEach(participantId => {
            const user = this.getUserById(participantId);
            if (user) {
                participantsHTML += `
    ...
    clearConversation(conversationId) {
        if (confirm('هل أنت متأكد من رغبتك في مسح جميع الرسائل في هذه المحادثة؟')) {
            const conversations = this.storage.getItem('conversations') || '[]';
            let conversationsList = JSON.parse(conversations);
            ...
            this.closeModal('conversationInfoModal');
            this.showSuccess('تم المسح', 'تم مسح جميع الرسائل بنجاح');
    ...
    deleteConversation(conversationId) {
        if (confirm('هل أنت متأكد من رغبتك في حذف هذه المحادثة؟')) {
            ...
            this.closeModal('conversationInfoModal');
            this.showSuccess('تم الحذف', 'تم حذف المحادثة بنجاح');
    ...
    async loadSettingsData() {
        try {
            const settingsContent = `
                <div class="settings-container">
                    <div class="settings-header">
                        ...
                        <button class="action-btn small" onclick="app.saveAllSettings()">
                            <i class="fas fa-save"></i>
                            حفظ جميع الإعدادات
                        </button>
                    </div>
            ...
            // إنشاء صفحة الإعدادات الديناميكية
            const settingsPage = document.createElement('div');
            settingsPage.id = 'settings';
            settingsPage.className = 'dashboard-section settings-section';
            settingsPage.innerHTML = settingsContent;
    ...
    getGeneralSettingsHTML() {
        return `
            <h3><i class="fas fa-palette"></i> التخصيص</h3>
            ...
            <div class="setting-item">
                ...
                <div class="theme-colors">
                    <div class="theme-color ${this.settings.theme === 'blue' ? 'active' : ''}" data-theme="blue" style="background: #0a9396;"></div>
                    <div class="theme-color ${this.settings.theme === 'purple' ? 'active' : ''}" data-theme="purple" style="background: #7209b7;"></div>
                    <div class="theme-color ${this.settings.theme === 'green' ? 'active' : ''}" data-theme="green" style="background: #2a9d8f;"></div>
                    <div class="theme-color ${this.settings.theme === 'orange' ? 'active' : ''}" data-theme="orange" style="background: #f3722c;"></div>
                </div>
            </div>
            ...
            <div class="setting-item">
                ...
                <select id="fontSize">
                    <option value="small" ${this.settings.fontSize === 'small' ? 'selected' : ''}>صغير</option>
                    <option value="medium" ${this.settings.fontSize === 'medium' ? 'selected' : ''}>متوسط</option>
            ...
            <div class="setting-item">
                ...
                <select id="language">
                    ...
                    <option value="en" ${this.settings.language === 'en' ? 'selected' : ''}>الإنجليزية</option>
                </select>
            </div>
            
            <h3><i class="fas fa-save"></i> الحفظ</h3>
            <div class="setting-item">
                <label>الحفظ التلقائي</label>
                <label class="switch">
                    <input type="checkbox" id="autoSaveToggle" ${this.settings.autoSave ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
            ...
            <div class="setting-item">
                ...
                <select id="autoSaveInterval">
                    <option value="1" ${this.settings.autoSaveInterval === 1 ? 'selected' : ''}>1 دقيقة</option>
                    <option value="5" ${this.settings.autoSaveInterval === 5 ? 'selected' : ''}>5 دقائق</option>
                    <option value="15" ${this.settings.autoSaveInterval === 15 ? 'selected' : ''}>15 دقيقة</option>
                    <option value="30" ${this.settings.autoSaveInterval === 30 ? 'selected' : ''}>30 دقيقة</option>
                </select>
            </div>
        `;
    }
    ...
    getNotificationSettingsHTML() {
        return `
            <h3><i class="fas fa-bell"></i> الإشعارات العامة</h3>
            <div class="setting-item">
            ...
            <div class="setting-item">
                <label>نغمة الإشعارات</label>
                <select id="notificationSound">
                    <option value="default" ${this.settings.notificationSound === 'default' ? 'selected' : ''}>افتراضي</option>
                    <option value="chime" ${this.settings.notificationSound === 'chime' ? 'selected' : ''}>نغمة</option>
                    <option value="bell" ${this.settings.notificationSound === 'bell' ? 'selected' : ''}>جرس</option>
                    <option value="none" ${this.settings.notificationSound === 'none' ? 'selected' : ''}>بلا صوت</option>
                </select>
            </div>
            ...
            <div class="setting-item">
                ...
                <label class="switch">
                    <input type="checkbox" id="emailMessages" ${this.settings.emailMessages ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
            ...
            <div class="setting-item">
                ...
                <label class="switch">
                    <input type="checkbox" id="emailProjectUpdates" ${this.settings.emailProjectUpdates ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
            
            <div class="setting-item">
                <label>العروض الجديدة</label>
                <label class="switch">
                    <input type="checkbox" id="emailOffers" ${this.settings.emailOffers ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
        `;
    ...
    getPrivacySettingsHTML() {
        return `
            <h3><i class="fas fa-user-shield"></i> الخصوصية</h3>
            <div class="setting-item">
                <label>الحساب الخاص</label>
                <label class="switch">
                    <input type="checkbox" id="privateAccount" ${this.settings.privateAccount ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
                <small>إخفاء ملفك الشخصي عن المستخدمين الآخرين</small>
            </div>
            
            <div class="setting-item">
                <label>عرض حالة الاتصال</label>
                <label class="switch">
                    <input type="checkbox" id="showOnlineStatus" ${this.settings.showOnlineStatus ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
            
            <div class="setting-item">
                <label>السماح بإرسال الرسائل</label>
                <select id="messagePermissions">
                    <option value="everyone" ${this.settings.messagePermissions === 'everyone' ? 'selected' : ''}>الجميع</option>
                    <option value="contacts" ${this.settings.messagePermissions === 'contacts' ? 'selected' : ''}>جهات الاتصال فقط</option>
                    <option value="none" ${this.settings.messagePermissions === 'none' ? 'selected' : ''}>لا أحد</option>
                </select>
            </div>
            
            <h3><i class="fas fa-history"></i> البيانات</h3>
            <div class="setting-item">
                <button class="action-btn outline" onclick="app.clearLocalData()">
                    <i class="fas fa-trash"></i>
                    مسح البيانات المحلية
                </button>
                <small>حذف جميع البيانات المحفوظة على جهازك</small>
            </div>
            
            <div class="setting-item">
                <button class="action-btn outline" onclick="app.exportData()">
                    <i class="fas fa-download"></i>
                    تصدير البيانات
                </button>
                <small>حفظ نسخة من بياناتك على جهازك</small>
            </div>
        `;
    }
    ...
    getAccountSettingsHTML() {
        return `
            ...
            <div class="account-info">
                <div class="profile-edit">
                    <div class="profile-image-large">
                        <img src="${this.currentUser?.avatar || 'https://ui-avatars.com/api/?name=مستخدم&background=0a9396&color=fff'}" 
                             alt="${this.currentUser?.name}" id="profileImagePreview">
                        <button class="edit-avatar-btn" onclick="app.changeProfilePicture()">
                    ...
                    <div class="profile-form">
                        <div class="form-group">
                            ...
                            <input type="text" id="fullName" value="${this.currentUser?.name || ''}">
                        </div>
                        <div class="form-group">
                            <label>البريد الإلكتروني</label>
                            <input type="email" id="userEmail" value="${this.currentUser?.email || ''}">
                        </div>
                        <div class="form-group">
                            <label>رقم الجوال</label>
                            <input type="tel" id="userPhone" value="${this.currentUser?.phone || ''}">
                        </div>
                        <button class="action-btn small" onclick="app.updateProfile()">
                            <i class="fas fa-save"></i>
                            حفظ التغييرات
                        </button>
                    </div>
            ...
            <div class="setting-item">
                <button class="action-btn outline" onclick="app.changePassword()">
            ...
            <div class="setting-item">
                ...
                <label class="switch">
                    <input type="checkbox" id="twoFactorAuth" ${this.settings.twoFactorAuth ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
            </div>
            
            <h3><i class="fas fa-sign-out-alt"></i> الجلسات</h3>
            <div class="setting-item">
                <button class="action-btn outline" onclick="app.logoutOtherSessions()">
            ...
            <div class="setting-item">
                <button class="action-btn outline danger" onclick="app.deleteAccount()">
                    <i class="fas fa-user-slash"></i>
                    حذف الحساب
                </button>
                <small>تحذير: لا يمكن التراجع عن هذا الإجراء</small>
            </div>
        `;
    }
    ...
    saveAllSettings() {
        try {
            ...
            this.settings = {
                ...
                autoSaveInterval: parseInt(document.getElementById('autoSaveInterval')?.value) || 5,
                notifications: document.getElementById('notificationsToggle')?.checked || true,
                notificationSound: document.getElementById('notificationSound')?.value || 'default',
                emailMessages: document.getElementById('emailMessages')?.checked || false,
                emailProjectUpdates: document.getElementById('emailProjectUpdates')?.checked || false,
                emailOffers: document.getElementById('emailOffers')?.checked || false,
                privateAccount: document.getElementById('privateAccount')?.checked || false,
                showOnlineStatus: document.getElementById('showOnlineStatus')?.checked || true,
                messagePermissions: document.getElementById('messagePermissions')?.value || 'everyone',
                twoFactorAuth: document.getElementById('twoFactorAuth')?.checked || false
            };
            
            this.saveSettings();
            this.showSuccess('تم الحفظ', 'تم حفظ جميع الإعدادات بنجاح');
    ...
    changeProfilePicture() {
        ...
        input.onchange = async (e) => {
            ...
            if (file) {
                try {
                    // محاكاة رفع الصورة
                    this.showLoading('جاري رفع الصورة...');
                    await new Promise(resolve => setTimeout(resolve, 1500));
                    
                    // إنشاء رابط محلي للصورة
                    ...
                    if (this.currentUser) {
                        ...
                        this.showSuccess('تم التغيير', 'تم تحديث صورة الملف الشخصي');
                    }
                } catch (error) {
                    this.showError('فشل التغيير', error.message);
                }
            }
        };
    ...
    updateProfile() {
        try {
            ...
            if (this.currentUser) {
                ...
                if (userPhone) this.currentUser.phone = userPhone;
                ...
                usersList = usersList.map(u => 
                    u.id === this.currentUser.id ? { ...u, ...this.currentUser } : u
                );
                this.storage.setItem('users', JSON.stringify(usersList));
                
                this.showSuccess('تم التحديث', 'تم تحديث الملف الشخصي بنجاح');
                this.updateUserInfo();
            }
    ...
    changePassword() {
        const modalContent = `
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="app.closeModal('changePasswordModal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button class="action-btn outline" onclick="app.closeModal('changePasswordModal')">
                    إلغاء
                </button>
                <button class="action-btn" type="submit" form="changePasswordForm">
        ...
        this.createModal('changePasswordModal', modalContent);
    ...
    async processPasswordChange() {
        try {
            ...
            const users = this.storage.getItem('users') || '[]';
            const usersList = JSON.parse(users);
            const user = usersList.find(u => u.id === this.currentUser?.id);
            ...
            this.storage.setItem('users', JSON.stringify(updatedUsersList));
            ...
            this.showSuccess('تم التغيير', 'تم تغيير كلمة المرور بنجاح');
    ...
    clearLocalData() {
        if (confirm('هل أنت متأكد من رغبتك في مسح جميع البيانات المحلية؟ هذا الإجراء لا يمكن التراجع عنه.')) {
            try {
                // مسح localStorage
                localStorage.clear();
                ...
                this.showSuccess('تم المسح', 'تم مسح جميع البيانات المحلية');
                ...
                this.showError('فشل المسح', error.message);
            }
        }
    }
    ...
    exportData() {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `pixel-art-backup-${new Date().toISOString().split('T')[0]}.json`;
            link.click();
            
            URL.revokeObjectURL(url);
            
            this.showSuccess('تم التصدير', 'تم حفظ نسخة من بياناتك على جهازك');
            
        } catch (error) {
            this.showError('فشل التصدير', error.message);
        }
    }
    
    logoutOtherSessions() {
        if (confirm('هل أنت متأكد من رغبتك في تسجيل الخروج من جميع الأجهزة الأخرى؟')) {
            this.showSuccess('تم التسجيل', 'تم تسجيل الخروج من جميع الأجهزة الأخرى');
        }
    }
    ...
    deleteAccount() {
        const modalContent = `
            <div class="modal-header">
                <h3>حذف الحساب</h3>
                <button class="modal-close" onclick="app.closeModal('deleteAccountModal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="warning-message">
                    ...
                    <p>سيؤدي حذف حسابك إلى:</p>
                    <ul>
                        <li>حذف جميع مشاريعك</li>
                        <li>حذف جميع رسائلك</li>
                        <li>فقدان رصيدك المالي</li>
                ...
                <form id="deleteAccountForm" style="margin-top: 20px;">
                    <div class="form-group">
                        <label>أدخل كلمة "حذف" للتأكيد *</label>
                        <input type="text" id="deleteConfirmation" placeholder="اكتب 'حذف' هنا" required>
                    </div>
                    <div class="form-group">
                        <label class="checkbox-label">
                            <input type="checkbox" id="deleteAgreement" required>
                            <span>أقر بأنني فهمت أن هذا الإجراء لا يمكن التراجع عنه</span>
                        </label>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="action-btn outline" onclick="app.closeModal('deleteAccountModal')">
                    إلغاء
                </button>
                <button class="action-btn danger" type="submit" form="deleteAccountForm">
                    <i class="fas fa-user-slash"></i>
                    نعم، احذف حسابي
                </button>
            </div>
        ...
        this.createModal('deleteAccountModal', modalContent);
    ...
    async processAccountDeletion() {
        try {
            ...
            if (confirmation !== 'حذف') {
                throw new Error('يرجى كتابة "حذف" للتأكيد');
            }
            ...
            // حذف حساب المستخدم من القائمة
            const users = this.storage.getItem('users') || '[]';
            let usersList = JSON.parse(users);
            usersList = usersList.filter(u => u.id !== this.currentUser?.id);
            this.storage.setItem('users', JSON.stringify(usersList));
            
            // حذف بيانات المستخدم المرتبطة
            this.deleteUserData(this.currentUser?.id);
            ...
            this.showSuccess('تم الحذف', 'تم حذف حسابك بنجاح');
    ...
    deleteUserData(userId) {
        ...
        const projects = this.storage.getItem('projects') || '[]';
        let projectsList = JSON.parse(projects);
        projectsList = projectsList.filter(p => p.userId !== userId);
        this.storage.setItem('projects', JSON.stringify(projectsList));
        
        // حذف معاملات المستخدم
        const transactions = this.storage.getItem('transactions') || '[]';
        let transactionsList = JSON.parse(transactions);
        transactionsList = transactionsList.filter(t => t.userId !== userId);
        this.storage.setItem('transactions', JSON.stringify(transactionsList));
        
        // حذف محادثات المستخدم
        const conversations = this.storage.getItem('conversations') || '[]';
        let conversationsList = JSON.parse(conversations);
        conversationsList = conversationsList.filter(c => !c.participants?.includes(userId));
        this.storage.setItem('conversations', JSON.stringify(conversationsList));
        
        // حذف إشعارات المستخدم
        const notifications = this.storage.getItem('notifications') || '[]';
        let notificationsList = JSON.parse(notifications);
        notificationsList = notificationsList.filter(n => n.userId !== userId);
        this.storage.setItem('notifications', JSON.stringify(notificationsList));
    }
    
    // ===== وظائف المستخدم النهائية =====
    ...
    logout() {
        if (confirm('هل أنت متأكد من رغبتك في تسجيل الخروج؟')) {
            try {
                ...
                this.showError('فشل تسجيل الخروج', 'حدث خطأ أثناء تسجيل الخروج');
            }
        }
    }
    ...
    async toggleOnlineStatus() {
        try {
            ...
            if (this.isOnline) {
                this.showSuccess('الاتصال', 'أنت الآن متصل');
                await this.syncPendingData();
            } else {
                this.showInfo('الاتصال', 'أنت الآن غير متصل');
            }
            
        } catch (error) {
            console.error('فشل تغيير حالة الاتصال:', error);
        }
    }
}
...
class PixelArtStorage {
    ...
    checkAvailability() {
        try {
            ...
            console.error('التخزين المحلي غير متاح:', error);
            return false;
        }
    }
    ...
    setItem(key, value) {
        ...
        try {
            ...
            console.error(`فشل حفظ المفتاح ${key}:`, error);
        }
    }
    ...
    getItem(key) {
        ...
        try {
            ...
            console.error(`فشل قراءة المفتاح ${key}:`, error);
            return null;
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // جعل التطبيق متاحاً عالمياً للاستدعاء من HTML
    window.app = app;
    
    // تهيئة service worker للوضع غير المتصل
    if ('serviceWorker' in navigator) {
...
// التعامل مع التثبيت على الشاشة الرئيسية
...
function toggleFullscreen() {
    if (!document.fullscreenElement) {
        document.documentElement.requestFullscreen().catch(err => {
            console.error(`فشل تفعيل وضع الشاشة الكاملة: ${err.message}`);
        });
    } else {
